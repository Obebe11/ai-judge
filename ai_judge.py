...
ВАЖНО: весь вердикт должен уложиться в 10 предложений суммарно (verdict + reasoning + advice + facts вместе ≤10 предложений). Пиши кратко, без воды. Если нужно — используй пункты, но общее число предложений ≤10.

Верни СТРОГО JSON без markdown:
{
  "winner": "Сторона A" | "Сторона B" | "Ничья" | "Недостаточно данных",
  "confidence": 0-100,
  "verdict": "1-2 предложения: кто прав и почему, кратко (входит в лимит 10)",
  "reasoning": "краткий разбор по пунктам, ≤5 предложений",
  "facts": ["список проверяемых фактов с указанием кто что утверждал и верно ли это (каждый факт — 1 предложение)"],
  "sources": ["список источников/типов источников для проверки, если применимо"],
  "advice": "совет как закрыть спор, 1 предложение"
}
"""
